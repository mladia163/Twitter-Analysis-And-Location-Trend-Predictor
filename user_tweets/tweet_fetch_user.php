...
$myfile1 = fopen("C:/xampp/htdocs/my/finalminor/php_user_tweet_data.txt", "w");
fwrite($myfile1, "~~". $burr[1]);
fclose($myfile1);

echo '<form action="../php_python_php_user.php" method="POST">';
echo '<input type="submit" value="Python Lda">';
echo '<input type="hidden" value="'.$location.'" name="location">';
echo '</form>';

echo '<form action="../tweet_save_user.php" method="POST">';
